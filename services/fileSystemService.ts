...
export const generateProductTextFileContent = (p: ProductData): string => {
  return `PRODUCT INFORMATION SHEET
Generated by Kiosk Architect
================================================================================
...
--------------------------------------------------------------------------------
${p.specs.map(s => `${(s.key || '').padEnd(30, ' ')}: ${s.value}`).join('\n')}
...
export const saveToKioskLibrary = async (rootHandle: FileSystemDirectoryHandle, product: ProductData, media: MediaFiles) => {
  ...
  // 3b. info.txt (Full Text Record)
  const infoContent = generateProductTextFileContent(product);
  const infoFile = await productDir.getFileHandle("info.txt", { create: true });
  await writeFile(infoFile, infoContent);
