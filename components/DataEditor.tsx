...
const DataEditor: React.FC<DataEditorProps> = ({ data, onChange }) => {
  ...
  const inputClass = "w-full p-3 bg-black border border-white/10 rounded-xl focus:ring-1 focus:ring-purple-500 focus:border-purple-500 outline-none text-slate-200 transition-all placeholder:text-slate-700";
  const cardClass = "bg-[#111] rounded-3xl shadow-xl border border-white/5 p-8 relative overflow-hidden";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div className={cardClass}>
             <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-2">
                ...
                <button onClick={() => addArrayItem('keyFeatures')} className="text-[10px] bg-purple-900/30 text-purple-400 px-2 py-1 rounded hover:bg-purple-900/50 transition-colors uppercase font-bold tracking-wider">
                    + Add
                </button>
             </div>
             <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                {data.keyFeatures.map((feature, i) => (
                    <div key={i} className="flex gap-2 group">
                        ...
                        />
                        <button onClick={() => removeArrayItem('keyFeatures', i)} className="text-slate-600 hover:text-red-500 p-2 opacity-0 group-hover:opacity-100 transition-all">
                            <X className="w-3 h-3" />
                        </button>
                    </div>
                ))}
         ...
         <div className={cardClass}>
             <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-2">
                ...
                <button onClick={() => addArrayItem('whatsInTheBox')} className="text-[10px] bg-purple-900/30 text-purple-400 px-2 py-1 rounded hover:bg-purple-900/50 transition-colors uppercase font-bold tracking-wider">
                    + Add
                </button>
             </div>
             <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                {data.whatsInTheBox.map((item, i) => (
                    <div key={i} className="flex gap-2 group">
                        <input 
                            ...
                            className={`${inputClass} py-2 text-xs`}
                        />
                         <button onClick={() => removeArrayItem('whatsInTheBox', i)} className="text-slate-600 hover:text-red-500 p-2 opacity-0 group-hover:opacity-100 transition-all">
                            <X className="w-3 h-3" />
                        </button>
                    </div>
                ))}
                    ...
                    {data.specs.map((spec, i) => (
                        <div key={i} className="flex gap-2 items-center group">
                            <input 
                                ...
                                className={`${inputClass} w-1/3 py-2 text-xs font-bold text-purple-200/80`}
                            />
                            ...
                            <input 
                                ...
                                className={`${inputClass} w-2/3 py-2 text-xs`}
                            />
                            <button onClick={() => removeSpec(i)} className="text-slate-600 hover:text-red-500 p-1 opacity-0 group-hover:opacity-100 transition-all">
                                <X className="w-3 h-3" />
                            </button>
                        </div>
                    ))}
